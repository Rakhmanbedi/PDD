                        ...
                        <div class="payment-card shadow-lg" style="border-radius: 10px">
                            <div class="container">
                                ...
                                @if(Auth::user()->balance != null)
                                    {{__('message.Balance')}}: {{ Auth::user()->balance }} ₸
                                @endif
                                <form action="{{route('addBalance',\Illuminate\Support\Facades\Auth::user()->id)}}" method="post">
                                    @csrf
                                    @method('PUT')
                                    {{__('message.Add_')}}: <input type="text" name="balance" class="form-control">

                                    <button type="submit" class="btn btn-primary">{{__('message.Add_')}}</button>
                                </form>
